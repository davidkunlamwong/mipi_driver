...
#define VC_MIPI     1   /* CCC - camera_common.c - VC MIPI driver code */
...
#define TRACE_CAMERA_COMMON_TRY_FMT     0   /* DDD - camera_common_try_fmt() - trace */
#define TRACE_CROP_MODE                 1   /* DDD - camera_common_try_fmt() - trace crop mode */

    struct camera_common_data *s_data = to_camera_common_data(sd->dev);
    struct tegra_channel *chan = v4l2_get_subdev_hostdata(sd);
    struct v4l2_control hdr_control;
//    const struct camera_common_frmfmt *frmfmt = s_data->frmfmt;
    struct camera_common_frmfmt *frmfmt = (struct camera_common_frmfmt *)s_data->frmfmt;
...
#if TRACE_CAMERA_COMMON_TRY_FMT
    dev_err(sd->dev, "%s(): mf: code=0x%x  mf->width,mf->height=%d,%d numfmts=%d\n", __func__, mf->code, mf->width, mf->height, s_data->numfmts);
// mf: width,height passed by v4l2-ctl
#endif
...
#if TRACE_CAMERA_COMMON_TRY_FMT
    dev_err(sd->dev, "%s(): s_data: fmt_width,fmt_height=%d,%d\n", __func__, s_data->fmt_width, s_data->fmt_height);
#endif
...
#if TRACE_CAMERA_COMMON_TRY_FMT
        dev_err(sd->dev, "%s: use_sensor_mode_id=true %d, numfmts=%d\n",
                __func__, s_data->sensor_mode_id, s_data->numfmts);
#endif
...
#if TRACE_CAMERA_COMMON_TRY_FMT
        dev_err(sd->dev, "%s: use_sensor_mode_id=false (%d), numfmts=%d\n",
                __func__, s_data->sensor_mode_id, s_data->numfmts);
#endif

//#define VC_MIPI_CROP_FIX  1
//#if VC_MIPI_CROP_FIX

#if VC_MIPI     /* CCC - camera_common_try_fmt() - crop mode */
/*
* Crop mode supported by:
*   IMX392
*/
{
        unsigned char *sen_model = get_sensor_model();
//#error frmfmt[1] = crop fotmat
//        if(s_data->numfmts >= 2)

        if(!strcmp(sen_model, "imx392"))    // IMX392
        {
          int idx = 0;  // 1;
          int dx,dy;    // old dimensions

          dx = frmfmt[idx].size.width;   // save old dimensions
          dy = frmfmt[idx].size.height;
          frmfmt[idx].size.width  = mf->width;
          frmfmt[idx].size.height = mf->height;
#if TRACE_CROP_MODE
          if(dx != frmfmt[idx].size.width || dy != frmfmt[idx].size.height)
          {
            dev_err(sd->dev, "%s: %s: New mode: width=%d height=%d (old=%d,%d)\n", __func__, sen_model, mf->width, mf->height, dx,dy);
          }
#endif
        }
}
        ...
        for (i = 0; i < s_data->numfmts; i++) {
#if TRACE_CAMERA_COMMON_TRY_FMT
            dev_err(sd->dev, "%s: i=%d: frmfmt[i].size.width=%d frmfmt[i].size.height=%d\n", __func__, i, frmfmt[i].size.width, frmfmt[i].size.height);
            dev_err(sd->dev, "%s: i=%d: mf->width=%d mf->height=%d\n", __func__, i, mf->width, mf->height);
#endif
...
#if TRACE_CAMERA_COMMON_TRY_FMT
        dev_err(sd->dev, "%s: format idx=%d: Success\n", __func__, i);
#endif
                break;
            }
        }
...
#if TRACE_CAMERA_COMMON_TRY_FMT
            dev_err(sd->dev, "%s: Set: mf->width=%d mf->height=%d\n", __func__, mf->width, mf->height);
#endif
            goto verify_code;
        }
    ...
    {
        dev_err(sd->dev, "%s(): camera_common_verify_code() failed\n", __func__);
        err = -EINVAL;
    }
...
#define TRACE_CAMERA_COMMON_S_FMT     0   /* DDD - camera_common_s_fmt() - trace */
...
#if TRACE_CAMERA_COMMON_S_FMT
//    dev_err(sd->dev, "%s(): code=0x%x w,h=%d,%d\n", __func__, mf->code, mf->width, mf->height);
#endif

    /* MIPI CSI could have changed the format, double-check */
    if (!camera_common_find_datafmt(mf->code))
    {
        dev_err(sd->dev, "%s(): camera_common_find_datafmt() failed\n", __func__);
        return -EINVAL;
    }
...
#if TRACE_CAMERA_COMMON_S_FMT
    dev_err(sd->dev, "%s(): code=0x%x w,h=%d,%d pix_fmt=0x%x '%c%c%c%c'\n", __func__, mf->code, mf->width, mf->height,
                s_data->colorfmt->pix_fmt,
                (char)(s_data->colorfmt->pix_fmt & 0xff),
                (char)((s_data->colorfmt->pix_fmt >> 8) & 0xff),
                (char)((s_data->colorfmt->pix_fmt >> 16) & 0xff),
                (char)((s_data->colorfmt->pix_fmt >> 24) & 0xff));
#endif
...
#define TRACE_CAMERA_COMMON_G_FMT     0   /* DDD - camera_common_g_fmt() - trace */
...
#if TRACE_CAMERA_COMMON_G_FMT
    dev_err(sd->dev, "%s(): code=0x%x colorspace=%d w,h=%d,%d\n", __func__, fmt->code, fmt->colorspace, s_data->fmt_width, s_data->fmt_height);
#endif
