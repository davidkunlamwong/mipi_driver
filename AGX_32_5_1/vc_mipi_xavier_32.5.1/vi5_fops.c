...
#define VC_MIPI     1   /* CCC - VC MIPI code */
...
#if VC_MIPI
//#define CAPTURE_TIMEOUT_MS  (1*60*1000)  // 1 min   //0x7fffffff   // <0: wait forever
#define CAPTURE_TIMEOUT_MS  (2*60*1000)  // 2 min   //0x7fffffff   // <0: wait forever
//#define CAPTURE_TIMEOUT_MS  ((3*60*1000)-50*1000)  // 2 min, 10 sec  // OK
//#define CAPTURE_TIMEOUT_MS  ((3*60*1000)-30*1000)  // 2.5 min  : fails probably due to watchdog
//#define CAPTURE_TIMEOUT_MS  (3*60*1000)            // 3 min    : fails
#else
...
static char sensor_model[MAX_MODEL_LEN] = "";
...
EXPORT_SYMBOL(set_sensor_model);
...
/****** vi5_capture_enqueue = VI5 capture enqueue = 09.2020 *************/
static void vi5_capture_enqueue(struct tegra_channel *chan,
    struct tegra_channel_buffer *buf)
{

#define TRACE_VI5_CAPTURE_ENQUEUE   0   /* DDD - vi5_capture_enqueue - trace */
...
#if TRACE_VI5_CAPTURE_ENQUEUE
    dev_err(vi->dev, "%s(): buffer_index=%d\n", __func__, request.buffer_index);
#endif
...
/****** vi5_capture_dequeue = VI5 capture dequeue = 09.2020 *************/
static void vi5_capture_dequeue(struct tegra_channel *chan,
    struct tegra_channel_buffer *buf)
{

#define TRACE_VI5_CAPTURE_DEQUEUE   0   /* DDD - vi5_capture_dequeue - trace */
#define DISABLE_CAPTURE_CORR_ERR    0   /* DDD - vi5_capture_dequeue - disable capture correctable error */
...
#if TRACE_VI5_CAPTURE_DEQUEUE
    if(err)
    {
      dev_err(vi->dev, "%s(): vi_capture_status() err=%d\n", __func__, err);
    }
#endif
    if (err) {
        if (err == -ETIMEDOUT) {
...
#if TRACE_VI5_CAPTURE_DEQUEUE
        dev_err(vi->dev, "%s(): descr->status.status=%d != CAPTURE_STATUS_SUCCESS=%d, flags=0x%x\n", __func__,
              descr->status.status, CAPTURE_STATUS_SUCCESS, descr->status.flags);
#endif
...
#if VC_MIPI
          if(strcmp(get_sensor_model(), "imx183") &&
             strcmp(get_sensor_model(), "imx226") &&
             strcmp(get_sensor_model(), "imx250") &&
             strcmp(get_sensor_model(), "imx252") &&
             strcmp(get_sensor_model(), "imx273") &&
             strcmp(get_sensor_model(), "imx392") &&
             strcmp(get_sensor_model(), "imx296") )    // none of the above sensors
#endif
          {
#if !DISABLE_CAPTURE_CORR_ERR
            dev_warn(vi->dev,
...
/****** vi5_channel_start_streaming = VI5 start streaming = 09.2020 *****/
static int vi5_channel_start_streaming(struct vb2_queue *vq, u32 count)
{

#define TRACE_VI5_CHANNEL_START_STREAMING   0   /* DDD - vi5_channel_start_streaming - trace */
...
#if TRACE_VI5_CHANNEL_START_STREAMING
    dev_err(&chan->video->dev, "..................%s:..............\n", __func__);
#endif
