...
#define VC_CODE     1   /* CCC - new code for VC MIPI OV9281 camera driver - disable ECC check */
...
/****** csi4_stream_init = Stream init = 07.2019 ************************/
...
#define TRACE_CSI4_STREAM_CHECK_STATUS  0   /* DDD - csi4_stream_check_status() - trace */

    struct tegra_csi_device *csi = chan->csi;
    int status = 0;

#if TRACE_CSI4_STREAM_CHECK_STATUS
    dev_info(csi->dev, "%s()...\n", __func__);
//    msleep_range(1);
#endif
    dev_dbg(csi->dev, "%s\n", __func__);
...
#define TRACE_CSI4_START_STREAMING  0   /* DDD - csi4_start_streaming() - trace */
...
#if TRACE_CSI4_START_STREAMING
    dev_info(csi->dev, "%s() port_idx=%d, csi_lanes=%d\n", __func__, port_idx, csi_lanes);
#endif
...
#define TRACE_CSI4_STOP_STREAMING  0   /* DDD - csi4_stop_streaming() - trace */
