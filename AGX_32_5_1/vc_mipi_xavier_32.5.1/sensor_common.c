...
#define VC_MIPI                 1   /* CCC - sensor_common.c - VC MIPI driver code */
...
#define TRACE_EXTRACT_PIXEL_FORMAT  0   /* DDD - extract_pixel_format() - trace */
    ...
    if (strncmp(pixel_t, "bayer_bggr10", size) == 0)
        ...
        *format = V4L2_PIX_FMT_SRGGB8;                      // v4l2_fourcc('R', 'G', 'G', 'B') /*  8  RGRG.. GBGB.. */
//    else if (strncmp(pixel_t, "rgb444", size) == 0)
//        *format = V4L2_PIX_FMT_RGB444;                      // v4l2_fourcc('R', '4', '4', '4') /* 16  xxxxrrrr ggggbbbb */
//    else if (strncmp(pixel_t, "nv12", size) == 0)
//        *format = V4L2_PIX_FMT_NV12;                        // v4l2_fourcc('N', 'V', '1', '2') /* 12  Y/CbCr 4:2:0  */
//    else if (strncmp(pixel_t, "bayer_rggb10p", size) == 0)
//        *format = V4L2_PIX_FMT_SRGGB10P;
#endif
    else {
        pr_err("%s: vc_mipi: Need to extend format: %s\n", __func__, pixel_t);
        return -EINVAL;
    }

#if TRACE_EXTRACT_PIXEL_FORMAT
    pr_err("%s(): vc_mipi: Result format=0x%08x (%c%c%c%c)\n",
               __func__,*format,
               (char)((*format)     & 0xFF),
               (char)((*format>>8)  & 0xFF),
               (char)((*format>>16) & 0xFF),
